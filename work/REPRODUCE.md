...
# for the emulation and instrumentation layers as well:
.venv/bin/pip install unicorn frida==17.18.0 cryptography
```
...
blocked from this sandbox. `androguard` covers the DEX side. `unicorn` is needed by
`emu.py` and everything in step 10; `frida` **must** be 17.18.0 to match the Gadget
build the README tells you to download.)
...
## 10. Revision-4 probes

These produced §10 items 34–38 and §11.11. They all need `funcmap.json` and
`apk_extracted/` (steps 2 and 5 above) plus `unicorn` in the venv.

```bash
cd /home/user/apk/work

# reference AES decryptor; self-checks its Te0/Td0 against the .so's own tables
../.venv/bin/python analysis/aesdec.py

# func#36: capture rijndael_setup's arguments from inside the call
../.venv/bin/python analysis/keyrec36b.py
# func#36: input-length x second-argument sweep
../.venv/bin/python analysis/f36_derive.py
# func#36: determinism + stack pre-fill control
../.venv/bin/python analysis/f36_stackkey.py

# GCM key/nonce staging: run func#157/#158, search the heap for each encoding stage
../.venv/bin/python analysis/gcm_material.py
# func#30 re-verification + proof that the 0x1606a blob is dead
../.venv/bin/python analysis/f30_blob.py

# every Base64-hidden C string in .rodata, mapped to its referencing functions
../.venv/bin/python analysis/b64_all.py
../.venv/bin/python analysis/b64_sweep.py
../.venv/bin/python analysis/blob_hunt.py

# callees + data refs of one function, with labels
../.venv/bin/python analysis/callees.py 36 252 71
```

Two gotchas found the hard way in this pass:

* `x1` at `rijndael_setup` entry is **not always a raw key buffer**. For inputs
  longer than the libc++ SSO threshold (22 bytes) it points *at the data-pointer
  field* of a long `std::string`, so reading 32 bytes from it yields the struct's
  `cap`/`size` fields instead of key material. `keyrec36b.py` resolves this;
  `keyrec36.py` (kept for the record) does not, and its long-input rows are wrong.
* A hand-rolled AES **decryption** is easy to get subtly wrong: the round loop is
  `rnd = Nr-1 downto 0` with `AddRoundKey(rnd)` *inside* it and `InvMixColumns`
  skipped when `rnd == 0`, and the flat state index is `st[4*col + row] ==
  in[row + 4*col]`. Three earlier attempts in `f36_key.py` failed on those two
  points; `aesdec.py` is the version that passes the FIPS-197 AES-128/192/256
  decryption vectors and round-trips against `aesref.enc_block`.

## 11. The agent's live checks, with no device

`frida/selftest_real_so.js` backs `NativePointer.read*` with the real `.so` file and resolves
`MOD` through the agent's own `findModule()`, so the six *live* `selfTest` vectors (XOR-`0x5A`
decode of the AES-192 key and the two GCM nonces, the plaintext key, the S-box, the Rcon) and
`rpc.exports.signature()` can be re-verified from the repository root:
