 ...
 *      retval.replace() is never called anywhere in this file.
...
function hookDetection() {
    ...
    emit('boot', { what: 'detection watchers armed (log-only)',
                   count: D.length,
            ...
            Interceptor.attach(p, {
                ...
                onLeave(retval) {
                    ...
                    emit('file', {
                        ...
                        retvalNote: 'buffer UNMODIFIED — this script captures, it does not hide'
                    });
                }
            });
        } catch (e) {}
        ...
        try {
            Interceptor.attach(p, {
                ...
                onLeave(retval) {
                    ...
                    if (/su|magisk|superuser|busybox|daemonsu|frida|xposed|substrate|riru|\/proc\//i.test(this.path)) {
                        emit('file', { what: fn, path: this.path, retval: r,
                                       exists: r === 0,
                                       retvalNote: 'logged, NOT forced to ENOENT' });
                    }
                }
            });
        } catch (e) {}
    ...
    Java.perform(() => {
        ...
        if (CFG.captureNet) {
            try {
                ...
                CH.proceed.overload('okhttp3.Request').implementation = function (req) {
                    const t0 = Date.now();
                    let reqInfo = null;
                    try { reqInfo = dumpRequest(req); } catch (e) { reqInfo = { err: String(e) }; }
                    const resp = this.proceed(req);           /* untouched */
                    let respInfo = null;
        ...
        if (CFG.captureDigest) {
            try {
                ...
                mdGetInst.implementation = function (alg) {
                    ...
                    let r;
                    try { r = MD.getInstance(alg); } finally { mdGetInst.implementation = mdGetInst; }
                    return r;
                };
                ...
                mdUpdate.implementation = function (b) {
                    ...
                    let r;
                    try { r = this.update(b); } finally { mdUpdate.implementation = mdUpdate; }
                    return r;
                };
            ...
            try {
                ...
                b64dec.implementation = function (s, f) {
                    ...
                    let r;
                    try { r = B.decode(s, f); } finally { b64dec.implementation = b64dec; }
        ...
        if (CFG.capturePkg) {
            ...
            try {
                ...
                Sig.hashCode.implementation = function () {
                    const r = this.hashCode();
...
function install() {
    ...
    emit('boot', {
        ...
        file: SINK.path, readOnly: true,
        replacedNothing: 'no Interceptor.replace and no retval.replace anywhere in this script',
        excludedOnPurpose: 'func#224 @0x157628 (reachable `b .` trap @0x1576ac)',
        tip: 'rpc.exports.stats() then rpc.exports.keys() then rpc.exports.net(20)'
    });
}
