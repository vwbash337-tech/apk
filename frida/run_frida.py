...
SIMPLE = {
    ...
    'clear': 'clear',
    # capture (topfollow_capture.js) -- 25 rpc exports
    # (frida-python turns camelCase rpc names into snake_case attributes;
    #  verified against the actual rpc.exports block in topfollow_capture.js)
    'cfg': 'cfg', 'stats': 'stats', 'endpoints': 'endpoints', 'events': 'events',
    'tail': 'tail', 'find': 'find', 'net': 'net', 'gcm': 'gcm',
    'strings': 'strings', 'nestedstrings': 'nested_strings',
    'keyschedule': 'keyschedule', 'cbc': 'cbc', 'blocks': 'blocks',
    'regnatives': 'reg_natives', 'onload': 'onload', 'blobs': 'blobs',
    'jnimap': 'jni_map', 'flush': 'flush', 'dumpall': 'dump_all',
}
...
HELP = """
  ...
  --- capture (topfollow_capture.js) ---
  cfg | stats | tail | flush | dumpall | events <kind> [n] | find <needle>
  keys | keyschedule | cbc | blocks | crypto | gcm        (poora AES chain)
  strings | nestedstrings | blobs                          (string layer)
  net | endpoints                                          (request/response)
  jni | jnimap | regnatives | onload | detect | calibrate

  Ek command us script par jaata hai jiske paas wo export hai; dono inject ho
  (--also) to agent ke export (selftest/calibrate/signature/...) agent par,
  baaki capture par. `help` dobara. quit
"""


async def repl(script, sink, script2=None):
    ...
    if script2 is not None:
        ex2 = script2.exports
        try:
            avail2 = script2.list_exports_sync()
        except Exception:
            avail2 = []

    def resolve(name):
        """(exports, source) for a snake_case rpc name, searching both scripts."""
        for e, av, tag in ((ex, avail, 'primary'), (ex2, avail2, 'also')):
            if e is None:
                continue
            if name in av or hasattr(e, name):
                return getattr(e, name), tag
        return None, None

    loop = asyncio.get_event_loop()
    log('[rpc] primary: %d exports: %s' % (len(avail), ', '.join(sorted(avail))))
    if ex2 is not None:
        log('[rpc] also   : %d exports: %s' % (len(avail2), ', '.join(sorted(avail2))))
    ...
    while True:
        ...
        try:
            if cmd == 'decrypt':
                ...
                # agent: self_test / self_calibrate   capture: selfTest / calibrate
                fn = None
                for nm in (('self_test', 'selfTest') if cmd == 'selftest'
                           else ('self_calibrate', 'calibrate')):
                    fn, src_tag = resolve(nm)
                    if fn is not None:
                        break
                if fn is None:
                    log('  [error] %s kisi script mein nahi hai' % cmd)
                    continue
                if ex2 is not None:
                    log('  [%s]' % src_tag)
                r = await call(fn)
                ...
                fn, src_tag = resolve(SIMPLE[cmd])
                if fn is None:
                    log('  [error] kisi bhi script mein %r export nahi hai' % SIMPLE[cmd])
                    continue
                if ex2 is not None:
                    log('  [%s]' % src_tag)
                r = await call(fn)
...
async def run(args):
    ...
    if args.bypass:
        ...
        cfg['stubDetection'] = True       # agent wala naam, dono script cover ho jaate hain
    ...
    # doosri script usi session mein. Frida har script ko alag JS context deta
    # hai lekin process share hota hai, isliye agent ke Interceptor.replace()
    # detectors capture script ke Interceptor.attach() ko nahi todte -- attach
    # entry par lagta hai, replace poore body ki jagah leta hai. Order matter
    # karta hai: bypass pehle arm ho, capture baad mein.
    script2 = None
    if args.also:
        with open(args.also, 'r', encoding='utf-8') as f2:
            src2 = f2.read()
        log('[also] %s' % args.also)
        script2 = await session.create_script(runtime + src2)
        script2.on('message', sink.on_message)
        await script2.load()
    if spawned:
    ...
    if args.rpc:
        ...
        # --also ke saath RPC us script par jaata hai jisne bypass kiya ho,
        # warna primary par. selftest/calibrate agent ke hain, stats/cfg capture ke.
        ex = script.exports
        if script2 is not None:
            AGENT_RPCS = {'selftest', 'calibrate', 'signature', 'vectors',
                          'knownkeys', 'xordecode', 'decrypt', 'secrets', 'notes'}
            if args.rpc.replace('-', '_') in AGENT_RPCS and args.also == AGENT:
                ex = script2.exports
            elif args.rpc.replace('-', '_') not in AGENT_RPCS and args.script == AGENT:
                ex = script2.exports
    ...
    await asyncio.sleep(args.wait)
    await repl(script, sink, script2)
    sink.save()
    for sc in (script2, script):
        if sc is None:
            continue
        try:
            await sc.unload()
...
def main():
    ...
    ap.add_argument('--bypass', action='store_true',
                    help='detection bypass ON karo. Bina iske defended device par app chalegi '
                         'hi nahi: repack APK func#226/#73 ka signature pin fail karta hai, '
                         'aur func#99/#162/#200/#225/#60 maps mein Frida dekh lete hain, '
                         'func#169/#154 root. Capture ke liye lagao; read-only evidence ke '
                         'liye mat.')
    ap.add_argument('--also', default=None,
                    help='ek aur script usi session mein inject karo. Useful combo: '
                         '--script capture --also agent --mode bypass (agent bypass karta '
                         'hai, capture sirf observe karta hai -- 138-tested code, no '
                         'duplication). agent | capture | <path>')
    ...
    a = ap.parse_args()
    for attr in ('script', 'also'):
        v = getattr(a, attr)
        if not v:
            continue
        if v in SCRIPT_CHOICES:
            v = SCRIPT_CHOICES[v]
        elif not os.path.isabs(v):
            v = os.path.join(HERE, v)
        if not os.path.isfile(v):
            ap.error('%s nahi mila: %s' % (attr, v))
        setattr(a, attr, v)
    if a.bypass and a.script != CAPTURE:
        log('[warn] --bypass sirf capture script ke CFG.bypass ko set karta hai; '
            'agent ke liye --mode bypass istemaal karo')
